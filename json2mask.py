...
Files_in_directory = []
for root, dirs, files in os.walk("reshaped_images"):
...
for j in range(len(all_file_names)): 
    ...
    if image_name in Files_in_directory: 
         img = np.asarray(PIL.Image.open('reshaped_images/'+image_name))    
    else:
        continue
    ...
    if data[all_file_names[j]]['regions'] != {}:
        ...
        output_dir= os.path.join(os.getcwd(),"masks")
        if(not os.path.exists(output_dir)):
            os.mkdir(output_dir)
        output_name,ext=image_name.split(".")
        cv2.imwrite('masks/'+output_name+"."+ext,mask.astype(np.uint8))